...
7 variables in common between the datasets

## Replicability and reproducibility

```{r echo=FALSE, out.width = "100%"}
include_graphics("../docs/git_slide.png")
```
...
```{r echo=FALSE, out.width = "100%"}
include_graphics("../docs/join_slide.png")
```

## PCA and Clustering 
We calculated the principal components using prcomp
```{r,  eval = FALSE}
# Obtain components
...
```
And we verify that the PC1 and PC2 account for the most variation in the data
```{r,  eval = FALSE}
# Calculate the variance explained by each component
variance_explained <- prostate_pca %>% 
  tidy("pcs")
```
Then we perform a clustering using kmeans
...
## Classification
We used keras Tensorflow to classify the data basing on the "cat_status" variable
```{r eval = FALSE}
