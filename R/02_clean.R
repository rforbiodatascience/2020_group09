...
col_names <- c("Patient_ID", "Stage", "Estrogen(mg)", 
...
prostate_data <- prostate_data %>%
  mutate('Estrogen(mg)' =str_replace(string = "Estrogen(mg)", pattern = "placebo", replacement = "0.0")) %>%  
...
TCGA_prostate_def <- TCGA_prostate_tot %>% 
  ...
  select("Patient_ID" = patient_id, age_at_initial_pathologic_diagnosis, bone_scan_results, OS.time, Dataset, SG) 
  
# Remove duplicated rows
TCGA_prostate_def <- TCGA_prostate_def %>%
  unique()

# Transform data format to alow join
prostate_data %>%
...
TCGA_prostate_def <- TCGA_prostate_def %>% 
  ...
  select(Patient_ID, age_at_initial_pathologic_diagnosis, Months_FU, bone_scan_results, Dataset, SG) 
...
TCGA_prostate_final <- TCGA_prostate_def %>%
  select(Patient_ID, age_at_initial_pathologic_diagnosis, Months_FU, Bone_metastases, Dataset, SG)


colnames(TCGA_prostate_final) <- c("Patient_ID", "Age", "Months_FU", "Bone_metastases", "Dataset", "SG")

prostate_tidy <- full_join(x = prostate_data, y = TCGA_prostate_final, by = c("Patient_ID", "Age", "Months_FU", "Dataset", "Bone_metastases", "SG")) %>% 
